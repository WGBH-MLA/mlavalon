...
module Avalon
  module Batch
    class HybridManifest < Manifest
      ...
      def path_to(f)
        # Uses path_to_files from Settings so that we can use local CSV manifest
        # with a remote file location.
        File.join(Settings.dropbox.path_to_files,f)
      end
