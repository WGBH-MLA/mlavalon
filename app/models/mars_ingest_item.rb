...
class MarsIngestItem < ActiveRecord::Base
  ...
  #   @fileset_indexes ||= csv_header_array.each_with_index.map { |header, i| i if MarsManifest.initial_file_header?(header) }.compact
  # end

  # def pull_filesets(indexes)
  #   @filesets ||= [].tap do |all_filesets|
  #     # indexes is array of filesetstart indexes
  #     fileset_indexes.each_with_index do |start_of_fileset, index|
  #       fileset = { 'files' => [{}] }
  #
  #       # cut out section for this fileset
  #       start_of_next_fileset = fileset_indexes[index + 1] || -1
  #
